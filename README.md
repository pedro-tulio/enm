...
- [Elastic Network Model Analysis](#elastic-network-model-analysis)
  ...
  - [Output Files](#output-files)
    ...
    - [Mode Data Files](#mode-data-files)
  ...
  - [Analysis](#analysis)
    ...
    - [Dynamical Cross-Correlation Matrix](#dynamical-cross-correlation-matrix)
  - [Usage Examples](#usage-examples)
    ...
    - [Write modes 7 through 10, plus modes 34 and 44 through 50  (vectors + trajectories)](#write-modes-7-through-10-plus-modes-34-and-44-through-50--vectors--trajectories)
    - [Write only the vector files for modes 26 and 41, skip trajectories](#write-only-the-vector-files-for-modes-26-and-41-skip-trajectories)
...
- **PDB File:** A protein structure file in PDB format (**`-i`** or **`--input`**). This argument is required for a normal ENM computation.
...
- **Write Modes:** Write vectors and/or trajectories for a specific set of modes from a previous run (**`-w`** or **`--write_modes`**). Use **`--no_nm_trj`** to skip trajectory writing and output only vectors.

### Feature Flags

- **`--no_nm_vec`**: Disable writing mode vectors

- **`--no_nm_trj`**: Disable writing mode trajectories. When used together with **`-w`**, only mode vector files (`.xyz`) are written and trajectory files (`_traj.pdb`) are skipped.
...
### Structure Files
- **`*_ca_structure.pdb`** or **`*_heavy_structure.pdb`**: Simplified structure file containing only the selected atoms

### Mode Data Files
- **`*_frequencies.npy`**: NumPy array containing mode frequencies

- **`*_modes.npy`**: NumPy array containing the full eigenvector matrix (shape `3N × M`). Together with `*_frequencies.npy` and the structure PDB, this file is all that is needed to write additional modes at any later point using **`-w`**.

- **`*_mode_*.xyz`**: XYZ files containing mode eigenvectors for individual modes, one file per mode
...
### Write modes 7 through 10, plus modes 34 and 44 through 50  (vectors + trajectories)
```
python enm.py -o results \
              -w 7:10,34,44:50
```

### Write only the vector files for modes 26 and 41, skip trajectories
```
python enm.py -o results \
              -w 26,41 \
              --no_nm_trj
