...
def parse_arguments():
    ...
    parser.add_argument('--no_nm_vec', action='store_true',
                       help='Disable writing mode vectors')
    parser.add_argument('--no_nm_trj', action='store_true',
                       help='Disable writing mode trajectories')
    parser.add_argument('--no_collectivity', action='store_true',
                       help='Disable collectivity calculation')
    parser.add_argument('--no_contributions', action='store_true',
                       help='Disable mode contributions plot')
    parser.add_argument('--no_rmsf', action='store_true',
                       help='Disable RMSF plot')
    parser.add_argument('--no_dccm', action='store_true',
...
def _create_ca_system(pdb_file, cutoff, spring_constant, output_prefix):
    ...
    print("\nCreating Cα-only system using Elastic Network Model...")
    ...
    n_atoms = len(ca_info)
    print(f"Selected {n_atoms} Cα atoms")
    ...
    print(f"Added {len(bonds)} ENM bonds with cutoff={cutoff}Å, min_distance=2.9Å, k={spring_constant} kcal/mol/Å²")
    ...
    print(f"C-alpha structure saved to {ca_pdb_file}")
...
def _create_heavy_system(pdb_file, cutoff, spring_constant, output_prefix):
    ...
    print("\nCreating heavy-atom system using Elastic Network Model...")
    pdb = app.PDBFile(pdb_file)
    ...
    system.addForce(enm_force)
    print(f"Added {len(bonds)} ENM bonds with cutoff={cutoff}Å, min_distance=2.0Å, k={spring_constant} kcal/mol/Å²")
    system.addForce(mm.CMMotionRemover())
    ...
    with open(heavy_pdb_file, 'w') as f:
        app.PDBFile.writeFile(new_topology, positions_quantity, f)
    print(f"Heavy-atom structure saved to {heavy_pdb_file}")
...
def hessian_enm(system, positions):
    ...
    print(f"\nComputing Hessian for {n_particles} particles...")
    ...
    print(f"ENM Hessian computed in {duration:.2f} seconds")
...
def compute_normal_modes(hessian, n_modes=None, use_gpu=False):
    ...
    if use_gpu and cp.is_available():
        print("Diagonalizing Hessian using GPU acceleration...")
        try:
            eigenvalues, eigenvectors = gpu_diagonalization(hessian, n_modes)
        except Exception as e:
            print(f"GPU diagonalization failed: {e}. Falling back to CPU.")
            use_gpu = False

    if not use_gpu or not cp.is_available():
        # CPU diagonalization with optimized parameters
        if n_modes is not None:
            print("Diagonalizing Hessian using CPU optimization...")
            ...
            print("Diagonalizing Hessian using CPU...")
            eigenvalues, eigenvectors = eigh(
                hessian,
                driver='evr',       # Fastest driver for symmetric matrices
                overwrite_a=True,
    ...
    duration = time.time() - start_time
    print(f"Diagonalization completed in {duration:.2f} seconds")
...
def write_nm_trajectory(topology, positions, mode_vector, output_file, system, model_type, amplitude=4, num_frames=34):
    """
    Write a PDB trajectory showing the motion along a normal mode.
    ...
    u = mode_vector * inv_sqrt_m

    rms = np.linalg.norm(u) / np.sqrt(n_particles)
    if rms < 1e-10:
        print("Skipping near-zero mode")
        return
...
def write_nm_vectors(modes, frequencies, system, topology, output_prefix, n_modes=10, start_mode=7):
    """
    Write normal mode eigenvectors to separate text files.
    ...
    if n_modes <= 0:
        print("No modes available to write.")
        return

    # Determine which PDB file to read based on the topology
    pdb_file = f"{output_prefix}_structure.pdb"

    # Read the PDB file to extract columns 2-6
    pdb_data = []
    with open(pdb_file, 'r') as f:
        for line in f:
            if line.startswith('ATOM') or line.startswith('HETATM'):
                # Extract columns 2-6 from the PDB line
                atom_number = line[6:11].strip()  # Columns 7-11 (1-indexed)
                atom_name = line[12:16].strip()   # Columns 13-16
                res_name = line[17:20].strip()    # Columns 18-20
                chain_id = line[21:22].strip()    # Column 22
                res_number = line[22:26].strip()  # Columns 23-26
    ...
    for mode_idx in range(start_mode, start_mode + n_modes):
        ...
        output_file = f"{output_prefix}_mode_{mode_number}.txt"

        with open(output_file, 'w') as f:
            # Write header
            f.write("# Normal Mode Eigenvectors\n")
            f.write(f"# System with {n_particles} particles\n")
            f.write(f"# Mode {mode_number} (Frequency: {freq:.2f} cm⁻¹)\n")
            ...
            for i in range(n_particles):
                if i < len(pdb_data):
                    ...
                    # Format according to specifications using data from PDB
                    f.write(f"{pdb_info['atom_number']:>5s}  {pdb_info['atom_name']:<3s} {pdb_info['res_name']:3s} "
                           f"{pdb_info['chain_id']:1s}{pdb_info['res_number']:>4s}  "
                           f"{x:14.10f} {y:14.10f} {z:14.10f}\n")
                else:
                    # Fallback if PDB data doesn't match
                    x, y, z = mode_vector[i]
                    f.write(f"{i+1:5d}  XXX UNK X    0     "
                           f"{x:13.10f} {y:13.10f} {z:13.10f}\n")
...
def write_collectivity(frequencies, modes, system, output_file, n_modes=20):
    ...
    print(f"\nSaved collectivity data to {output_file}")
...
def plot_mode_contributions(eigenvalues, output_file=None, n_modes=10):
    ...
    if output_file:
        ...
        print(f"\nSaved mode contribution plot to {output_file}")
    else:
        plt.show()
...
def plot_atomic_fluctuations(system, eigenvalues, modes, topology, output_file=None, temperature=300, n_modes=None, start_mode=6):
    ...
    if output_file:
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        print(f"\nResidue RMSF plot saved to {output_file}")
    else:
        plt.show()
...
def plot_residue_cross_correlation(system, eigenvalues, modes, topology, output_file=None, temperature=300, n_modes=None, start_mode=6, use_gpu=True, use_multithreading=True):
    ...
    if use_gpu and cp.is_available():
        print("\nUsing GPU acceleration for DCCM calculation...")
        try:
    ...
    if not use_gpu or not cp.is_available():
        print("\nUsing CPU for DCCM calculation...")
    ...
    duration = time.time() - start_time
    print(f"DCCM calculation completed in {duration:.2f} seconds")
    ...
    if output_file:
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        print(f"Residue dynamical cross-correlation plot saved to {output_file}")
...
def main():
    ...
    CONFIG = {
        ...
        "WRITE_NM_VEC": not args.no_nm_vec,
        "WRITE_NM_TRJ": not args.no_nm_trj,
        "COLLECTIVITY": not args.no_collectivity,
        "PLOT_CONTRIBUTIONS": not args.no_contributions,
        "PLOT_RMSF": not args.no_rmsf,
        "PLOT_DCCM": not args.no_dccm,
    ...
    print("Starting Normal Mode Analysis...")
    print(f"Input file: {input_file}")
    print(f"Output folder: {output_folder}")
    print(f"Mode: {'Cα-only ENM' if CONFIG['MODEL_TYPE'] == 'ca' else 'Heavy-atom ENM'}")
    print(f"Cutoff: {CONFIG['CUTOFF'] or ('15.0Å' if CONFIG['MODEL_TYPE'] == 'ca' else '12.0Å')}")
    print(f"Spring constant: {CONFIG['SPRING_CONSTANT']} kcal/mol/Å²")
    print(f"Number of modes to output: {CONFIG['OUTPUT_MODES']}")
    ...
    try:
        ...
        print(f"Results saved to {output_prefix}_{prefix}_*.npy files")
        ...
        if CONFIG["WRITE_NM_VEC"]:
            ...
            print(f"\nWriting vectors for {num_modes} modes...")
        ...
        if CONFIG["WRITE_NM_TRJ"]:
            print(f"\nGenerating trajectories for {num_modes} modes...")
            num_modes = min(CONFIG["OUTPUT_MODES"], len(frequencies)-6)

            for mode_idx in range(6, 6+num_modes):
                output_file = f"{output_prefix}_{prefix}_mode_{mode_idx+1}_traj.pdb"
                write_nm_trajectory(
                    topology, positions, modes[:, mode_idx], output_file, system,
                    CONFIG["MODEL_TYPE"]
                )
